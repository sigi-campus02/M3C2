...
This module provides :func:`calc_stats` for computing descriptive statistics
for a distance array and :func:`_load_params` which reads the normal and search
scales from a ``*_m3c2_params.txt`` file.  The implementation was previously
part of :mod:`statistics_service` but has been extracted into this dedicated
module to keep responsibilities focussed.
"""
...
def calc_stats(
    ...
    params_path : Optional[str], optional
        Path to a ``*_m3c2_params.txt`` file from which the normal and search
        scale are loaded.
    ...
    min_expected : Optional[float], optional
        Minimum expected count per histogram bin. If provided it is passed to
        the distribution fitting routine.
    tolerance : float, default 0.01
        Absolute distance threshold used for several ratio metrics.
    outlier_multiplicator : float, default 3.0
        Multiplicative factor applied to the chosen outlier metric to derive an
        inlier threshold.
    ...
    -------
    Dict
        A mapping with numerous aggregated statistics. The dictionary contains
        overall counts and sums, summary metrics (mean, median, RMS, standard
        deviation, MAE, NMAD), inlier/outlier information, quantiles and
        interquartile ranges for all data and inliers only, goodness-of-fit
        parameters for Gaussian and Weibull distributions as well as higher
...
def _load_params(params_path: Optional[str]) -> Tuple[float, float]:
    ...
    and ``"SearchScale="``.  When no file is supplied or a value is missing the
    respective scale defaults to ``numpy.nan``.

    Parameters
    ----------
    params_path:
        Path to the parameter file.  If ``None`` or the file does not exist,
        ``numpy.nan`` is returned for both values.
...
__all__ = ["calc_stats", "_load_params"]
