...
def exclude_outliers(data_folder, ref_variant, outlier_rmse_multiplicator=3, method="rms"):
    ...
    if method == "rms":
        ...
        outlier_mask = np.abs(distances_valid - mu) > (outlier_rmse_multiplicator * std)
        logger.info(f"[Exclude Outliers] STD: {std:.6f}")
        logger.info(f"[Exclude Outliers] Outlier-Schwelle: {outlier_rmse_multiplicator} * STD = {outlier_rmse_multiplicator * std:.6f}")
    elif method == "nmad":
        med  = np.median(distances_valid)
        nmad = 1.4826 * np.median(np.abs(distances_valid - med))
        outlier_mask = np.abs(distances_valid - med) > (outlier_rmse_multiplicator * nmad)
    else:
